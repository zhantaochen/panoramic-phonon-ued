...
class OdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(ctx, shapes, func, y0, t, rtol, atol, method, options, event_fn, adjoint_rtol, adjoint_atol, adjoint_method,
        ...
        with torch.no_grad():
            _ans = odeint(func, y0, t, rtol=rtol, atol=atol, method=method, options=options, event_fn=event_fn)

            if isinstance(_ans, tuple):
                ans = _ans[0]
                ctx.y_grid_list = _ans[1]
                ctx.time_grid_list = _ans[2]
                del _ans
            else:
                ans = _ans
                ctx.y_grid_list = None
                ctx.time_grid_list = None
    ...
    def backward(ctx, *grad_y):
        with torch.no_grad():
            ...
            # for i, _grad_y in enumerate(grad_y):
            #     print(i)
            #     print(_grad_y[0,:,0,0,-1])
            ...
            aug_state = [torch.zeros((), dtype=y.dtype, device=y.device), y[-1], torch.zeros_like(grad_y[-1])]  # vjp_t, y, vjp_y
            ...
            if 'requires_grad' in adjoint_options.keys(): adjoint_options['requires_grad'] = True
            if 'save_all_timepoints' in adjoint_options.keys(): adjoint_options['save_all_timepoints'] = False
            ...
            def augmented_dynamics(t, y_aug):
                ...
                with torch.enable_grad():
                    ...
                    if DEBUG_MODE:
                        '''debug'''
                        print(f"time: {t}    ", f"grad_outputs: {adj_y.shape}\n", adj_y[0,:,0,0,-1])
                        print("Inputs to autograd:")
                        # print(f"func_eval: {func_eval.shape}\n", func_eval[0,:,0,0,-1])
                        print(f"y: {y.shape}\n", y[0,:,0,0,-1])
                        # print(f"adjoint_params: {adjoint_params[0].shape}\n", adjoint_params[0])
                        '''debug'''
            ...
            for i in range(len(t) - 1, 0, -1):
                ...
                aug_state[2] += grad_y[i]# update any gradients wrt state at this time point
                if DEBUG_MODE:
                    '''debug'''
                    print(f"{i}-th dLdg: ", grad_y[i][0,:,0,0,-1], "\nlambda: \n",  aug_state[2][0,:,0,0,-1])
                    print("original dLdp: \n", aug_state[3])
                    '''debug'''
                # Run the augmented system backwards in time.
                # feed in the forward-pass y for numerical stability
                adjoint_options['true_states'] = {
                    'time_grid': -ctx.time_grid_list[i-1].flip((0,)), 
                    'y_grid': ctx.y_grid_list[i-1].flip((0,))
                }
                # print(i-1, adjoint_options['true_states']['time_grid'], adjoint_options['true_states']['y_grid'].shape)
                aug_state = odeint(
        ...
        del ctx.time_grid_list, ctx.y_grid_list
